...
```{r}
plasma_miRNAs <- data.frame(sample_fraction = (rowSums(plasma_counts > 0) / length(rownames(plasma_samples))))
plasma_miRNAs$Source <- "Plasma"
serum_miRNAs <- data.frame(sample_fraction = (rowSums(serum_counts > 0) / length(rownames(serum_samples))))
serum_miRNAs$Source <- "Serum"
detected_miRNAs <- rbind(plasma_miRNAs, serum_miRNAs)
ggplot(detected_miRNAs, aes(sample_fraction, fill=Source)) + geom_histogram(alpha=0.5, position='identity') + ggtitle("miRNA detection") + xlab("Fraction of Samples") + ylab("Number of miRNAs detected")
```
