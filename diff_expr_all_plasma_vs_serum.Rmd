...
sample.info.plasma.dt <- data.table(plasma_samples, keep.rownames = FALSE)
sample.info.plasma.dt[, MT.Unique.ID:=sub("PlasmaPool", "", sub("^X", "", as.character(MT.Unique.ID)))]  # strip leading X from MT.Unique.ID
names(sample.info.plasma.dt)[names(sample.info.plasma.dt) == "MT.Unique.ID"] = "lab.libMethod.replicate"
...
g <- ggplot(est.prob.detected.n.miRs, aes( x=lib.method.simple, y=n.detected, fill=lib.method.simple)) + 
